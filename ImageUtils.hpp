...
void AlphaBlendColors(RGBA& BottomPixel, RGBA& TopPixel)
{
	//Calculate new Alpha:
...
void Chara4Mask(cv::Mat& surface, cv::Mat& mask)
{
	for (int y = 0; y < surface.rows; ++y) {
		for (int x = 0; x < surface.cols; ++x) {
			RGBA& px = *(RGBA*)&surface.data[y * surface.step + x * 4];
			RGBA& mask_px = *(RGBA*)&mask.data[y * mask.step + x * 4];
			px.A = px.A * (mask_px.R / 255.0f);
		}
	}
}
